  ...
  return (
    <>
      ...
      <MobileHeader
        countryCode={countryCode}
        isHome={isHome}
        title={<img src="https://i.imgur.com/xV5O6tO.png" alt="logo" className="w-[100px]" />}
        openCart={openCart}
        openMenu={openMenu}
      />
    </>
      ...
      <div className="flex items-center justify-start w-full gap-4">
        ...
        >
          ...
          <Input
            className={
              isHome
                ? 'border-primary/20 focus:border-primary/20 dark:focus:border-primary/20'
...
function DesktopHeader({countryCode, isHome, menu, openCart, title}) {
  ...
  const styles = {
    ...
    container: `${
      ...
      y > 50 && !isHome ? 'shadow-lightHeader ' : ''
    }hidden h-nav items-center sticky transition duration-300 z-40 top-0 justify-between w-full leading-none gap-8 px-12 py-8`,
  };

  return (
    <header role="banner" className={styles.container}>
      <div className="flex gap-12">
        {!isHome ? (<Link className={`font-bold`} to="/">
          {title}
        </Link>) : (
          <></>
        )}
